...
try {
  ...
  assert.match(appSource, /history-result-stamp--\$\{entry\.result\.toLowerCase\(\)\}/);
  ...
  assert.match(appCss, /\.game-result-overlay \.summary-container\s*\{[^}]*box-sizing:\s*border-box[^}]*overflow:\s*hidden/s);
  assert.match(appCss, /\.summary-synergy-item\s*\{[^}]*margin:\s*0/s);
  assert.match(appCss, /\.summary-identity \.difficulty-icon-img\s*\{[^}]*margin-right:\s*12px/s);
  assert.match(appCss, /\.summary-identity-stats\s*\{[^}]*gap:\s*clamp\(22px, 2\.8vw, 34px\)/s);
  ...
  assert.match(appCss, /\.history-result-stamp--win\s*\{[^}]*#35c66b/s);
  assert.match(appCss, /\.history-result-stamp--loss\s*\{[^}]*#ef5a5a/s);
