...
// 定義各階段資訊 (帝王版本)
const STAGES = [
    { name: 'Novice (新手期)', data: MODERN_NOVICE_OPPONENTS, id: 'Novice', minWin: 80, maxWin: 95 },
    { name: 'Intermediate (中期)', data: MODERN_INTERM_OPPONENTS, id: 'Intermediate', minWin: 60, maxWin: 80 },
    { name: 'Advanced (進階期)', data: MODERN_ADVANCED_OPPONENTS, id: 'Advanced', minWin: 45, maxWin: 65 },
    { name: 'Elite (四天王)', data: MODERN_ELITE_OPPONENTS, id: 'Elite', minWin: 30, maxWin: 50 },
    { name: 'Champion (冠軍)', data: MODERN_CHAMPION_OPPONENTS, id: 'Champion', minWin: 15, maxWin: 35 }
];
...
async function runStageBalanceSim() {
    console.log(`開始進行帝王版本 (Modern Edition) 敵人強度考驗...`);
    let report = `# 帝王版本：敵人強度平衡測試報告\n\n`;
    ...
    try {
        writeFileSync('./modern_balance_report.md', report);
        console.log(`\n測試完成！報告已產出：modern_balance_report.md`);
    } catch (err) {
