...
const STAGES = [
    ...
    { name: 'Intermediate (中期)', data: INTERM_OPPONENTS, id: 'Intermediate', minWin: 60, maxWin: 80 },
    { name: 'Advanced (進階期)', data: ADVANCED_OPPONENTS, id: 'Advanced', minWin: 45, maxWin: 65 },
    { name: 'Elite (四天王)', data: ELITE_OPPONENTS, id: 'Elite', minWin: 30, maxWin: 50 },
    { name: 'Champion (冠軍)', data: CHAMPION_OPPONENTS, id: 'Champion', minWin: 15, maxWin: 35 }
];
...
async function runStageBalanceSim() {
    console.log(`開始進行經典版本 (Classic Edition) 敵人強度考驗...`);
    let report = `# 經典版本：敵人強度平衡測試報告\n\n`;
    ...
    try {
        writeFileSync('./classic_balance_report.md', report);
        console.log(`\n測試完成！報告已產出：classic_balance_report.md`);
    } catch (err) {
